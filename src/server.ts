...
import { z } from 'zod';
import { apiClient, TribeunalAPIError } from './client/api-client.js';
...
import {
  ...
  SeekExpertOpinionSchema,
  PollStakeholdersSchema,
  ConsensusBuilderSchema,
  AnalyzeReadinessSchema,
  PredictOutcomeSchema,
  IdentifyBiasSchema,
  FindSimilarDecisionsSchema,
  CalculateConfidenceSchema,
  ScheduleReminderSchema,
  EscalateDeadlockSchema,
...
export function setupTools(server: Server): void {
  ...
  server.setRequestHandler(
    z.object({ method: z.literal('tools/call'), params: z.object({ name: z.string(), arguments: z.unknown() }) }),
    async (request) => {
      ...
      try {
        switch (toolName) {
          ...
          case 'decision_find_active': {
            const params = FindActiveDecisionsSchema.parse(args);
          ...
          case 'decision_get_status': {
            const params = GetDecisionStatusSchema.parse(args);
            const trial = await apiClient.getTrial(params.id);
            return {
              content: [{ type: 'text', text: JSON.stringify(trial, null, 2) }],
            };
          }

          case 'decision_start_process': {
            const params = StartDecisionProcessSchema.parse(args);
            // Map new parameters to legacy API
            const trial = await apiClient.createTrial({
              title: params.title,
              description: params.description,
              type: params.decisionType,
              juryType: params.participationType,
              sides: params.options,
              trialLength: params.timeframe,
              decisionRequirement: params.consensusRequired,
...
${JSON.stringify(trial, null, 2)}`,
          ...
          case 'decision_choose_option': {
            const params = ChooseOptionSchema.parse(args);
            const result = await apiClient.castVote(params.decisionId, params.optionId);
            return {
              content: [
                {
                  type: 'text',
                  text: `Choice recorded successfully!${params.reasoning ? ` Reasoning: ${params.reasoning}` : ''}\n${JSON.stringify(result, null, 2)}`,
                },
              ],
            };
          }
          ...
          case 'decision_check_consensus': {
            const params = CheckConsensusSchema.parse(args);
            const stats = await apiClient.getVoteStats(params.decisionId);
            return {
          ...
          case 'decision_gather_evidence': {
            const params = GatherEvidenceSchema.parse(args);
            const evidence = await apiClient.getTrialEvidence(params.decisionId);
            return {
          ...
          case 'decision_provide_info': {
            const params = ProvideSupportingInfoSchema.parse(args);
            const evidence = await apiClient.submitEvidence(params.decisionId, {
              content: params.content,
              type: params.infoType,
              sideId: params.supportsOption,
            });
            return {
              content: [
                {
          ...
          case 'tribeunal_search_trials': {
            const params = SearchTrialsSchema.parse(args);
            const results = await apiClient.searchTrials(params);
            return {
              content: [{ type: 'text', text: JSON.stringify(results, null, 2) }],
            };
          }
          ...
          case 'tribeunal_get_trial': {
            const params = GetTrialSchema.parse(args);
            const trial = await apiClient.getTrial(params.id);
            return {
              content: [{ type: 'text', text: JSON.stringify(trial, null, 2) }],
            };
          ...
          case 'tribeunal_create_trial': {
            const params = CreateTrialSchema.parse(args);
            const trial = await apiClient.createTrial(params);
            return {
              content: [
                {
                  type: 'text',
...
${JSON.stringify(trial, null, 2)}`,
          ...
          case 'tribeunal_list_evidence': {
            const params = ListEvidenceSchema.parse(args);
            const evidence = await apiClient.getTrialEvidence(params.trialId);
            return {
              content: [{ type: 'text', text: JSON.stringify(evidence, null, 2) }],
            };
          }
          ...
          case 'tribeunal_cast_vote': {
            const params = CastVoteSchema.parse(args);
            const result = await apiClient.castVote(params.trialId, params.sideId);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'tribeunal_revoke_vote': {
            const params = RevokeVoteSchema.parse(args);
            const result = await apiClient.revokeVote(params.trialId, params.voteId);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'tribeunal_get_vote_stats': {
            const params = GetVoteStatsSchema.parse(args);
            const stats = await apiClient.getVoteStats(params.trialId);
            return {
              content: [{ type: 'text', text: JSON.stringify(stats, null, 2) }],
            };
          }
          ...
          case 'tribeunal_submit_evidence': {
            const params = SubmitEvidenceSchema.parse(args);
            const evidence = await apiClient.submitEvidence(params.trialId, {
              content: params.content,
              type: params.type,
            ...
            return {
              content: [
                {
                  type: 'text',
                  text: `Evidence submitted successfully!\n${JSON.stringify(evidence, null, 2)}`,
                },
              ],
            };
          }
          ...
          case 'tribeunal_rate_evidence': {
            const params = RateEvidenceSchema.parse(args);
            const result = await apiClient.rateEvidence(params.evidenceId, params.rating);
            return {
              content: [
                {
                  type: 'text',
                  text: `Evidence rated successfully!\n${JSON.stringify(result, null, 2)}`,
                },
              ],
            };
          }
          ...
          case 'tribeunal_list_tribes': {
            const params = ListTribesSchema.parse(args);
            const tribes = await apiClient.listTribes(params);
            return {
              content: [{ type: 'text', text: JSON.stringify(tribes, null, 2) }],
            };
          }

          case 'tribeunal_get_tribe': {
            const params = GetTribeSchema.parse(args);
            const tribe = await apiClient.getTribe(params.id);
            return {
              content: [{ type: 'text', text: JSON.stringify(tribe, null, 2) }],
            };
          }

          case 'tribeunal_join_tribe': {
            const params = JoinTribeSchema.parse(args);
            const result = await apiClient.joinTribe(params.tribeId);
            return {
              content: [
                {
                  type: 'text',
          ...
          case 'tribeunal_leave_tribe': {
            const params = LeaveTribeSchema.parse(args);
            const result = await apiClient.leaveTribe(params.tribeId);
            return {
              content: [
                {
                  type: 'text',
                  text: `Successfully left tribe.\n${JSON.stringify(result, null, 2)}`,
                },
              ],
            };
          }
          ...
          case 'tribeunal_create_tribe': {
            const params = CreateTribeSchema.parse(args);
          ...
          case 'tribeunal_get_user': {
            ...
            const user = await apiClient.getUser(params.id);
            return {
