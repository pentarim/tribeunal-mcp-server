...
- It exposes the **same 33 tools** as the local stdio server — they share a
...
| `src/mcp-agent.ts` | `TribeunalMCP extends McpAgent` — registers the shared 33 tools, each routed through a per-user `TribeunalAPIClient` using `this.props.upstreamAccessToken`. |
